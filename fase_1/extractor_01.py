...
# -*- coding: utf-8 -*-
"""
extractor_01.py — Extractor acumulativo de listas de precios  [v01]
...
Cambios v01 respecto a extractor.py:
    _limpiar_precio (función global):
      - Caso 0 nuevo: precio formato PS México "miles.decimal.centavos"
        ...
        este formato para precios de cuatro cifras; el nuevo sí.
    ExtractorPS._filtrar:
      - Descarta filas donde `pag` contiene texto (p.ej. "PASEO", "TELEMARKETING")
        que en el nuevo formato aparecen alineadas cerca de la columna ID
        y se colaban como registros fantasma.
    ExtractorPS._detectar_encabezado:
      - Tolerancia Y ampliada de ±5 px a ±15 px al buscar Sug_credito.
      - Fallback adicional: busca cualquier token que contenga la
        subcadena de col_precio (útil ante variaciones tipográficas menores).
    ExtractorPS._merge_y:
      - Valor por defecto aumentado de 12 a 16 px para tolerar la mayor
        fragmentación vertical del nuevo layout de Price Shoes (marcas
        partidas en múltiples sub-filas).
    ExtractorPS.extraer:
      - Logging adicional: informa cuando una página no tiene palabras.
      - Retiene col_x entre páginas aunque la página actual no tenga
        encabezado propio (comportamiento ya existente, ahora documentado).

Compatibilidad:
    Totalmente compatible con config_price.json existente.
    No se requieren cambios en el config ni en ningún otro módulo.

Uso:
    python3 fase_1/extractor_01.py --config fase_1/config/config_price.json
...
def _limpiar_precio(val: str) -> str | None:
    ...
    # ── Normalizar a entero ──────────────────────────────────────────
    # Caso 0 (v01 — Price Shoes México): Miles con punto Y decimal con punto
...
class ExtractorPS:
    ...
    def __init__(self, config: dict, logger: logging.Logger):
        ...
        # merge_y v01: ampliado de 12 a 16 px para tolerar la mayor
        # fragmentación vertical que introduce el nuevo formato de Price Shoes
        # (letras de marca partidas en múltiples sub-filas más separadas).
    ...
    def _detectar_encabezado(self, words: list) -> dict | None:
        ...
        Mejoras v01:
        - Tolerancia Y ampliada a ±15 px para Sug_credito (antes ±5).
        - Búsqueda de precio también en rango ±15 px ANTES del y del encabezado,
          útil cuando Price Shoes separa el encabezado en sub-filas.
        - Si Sug_credito no aparece exacto, busca cualquier token que
          contenga "credito" o "Sug_c" (variaciones ortográficas menores).
        """
        filas_y = {}
        for w in words:
        ...
        for y, tokens in sorted(filas_y.items()):
            ...
            # Búsqueda ampliada de precio: ±15 px en Y (antes ±5)
    ...
    def _filtrar(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # v01: descartar filas donde pag contiene texto (son del encabezado/pie de página)
